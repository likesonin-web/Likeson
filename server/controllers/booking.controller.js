...
export const getConsultationsByBookingId = asyncHandler(async (req, res) => {
    try {
...
export const patchConsultationsByConsultationIdConfirm = asyncHandler(async (req, res) => {
    try {
...
export const patchConsultationsByConsultationIdAccept = asyncHandler(async (req, res) => {
    try {
      const dp = await DoctorProfile.findOne({ user: req.user._id }).select('_id').lean();
...
export const patchConsultationsByConsultationIdStart = asyncHandler(async (req, res) => {
    try {
      const consultation = await Consultation.findById(req.params.consultationId);
      if (!consultation) return res.status(404).json({ success: false, message: 'Consultation not found' });
      if (req.user.role === 'doctor') {
...
export const patchConsultationsByConsultationIdEnd = asyncHandler(async (req, res) => {
    try {
...
export const patchConsultationsByConsultationIdConsent = asyncHandler(async (req, res) => {
    try {
...
export const postConsultationsByConsultationIdChat = asyncHandler(async (req, res) => {
    try {
...
export const getConsultationsByConsultationIdJoinToken = asyncHandler(async (req, res) => {
    try {
      const consultation = await Consultation.findById(req.params.consultationId)
...
// GET '/tp/assigned'
export const getTpAssigned = asyncHandler(async (req, res) => {
    try {
...
export const getTpDriversAvailable = asyncHandler(async (req, res) => {
  try {
    const tp = await TransportPartner.findOne({ user: req.user._id }).select('_id').lean();
    if (!tp) return res.status(404).json({ success: false, message: 'Transport partner not found' });
...
export const patchByIdTpAssignDriver = asyncHandler(async (req, res) => {
  try {
...
export const patchByIdTpReassignDriver = asyncHandler(async (req, res) => {
  try {
...
// GET '/care/assigned'
export const getCareAssigned = asyncHandler(async (req, res) => {
    try {
...
export const patchByIdCareArrived = asyncHandler(async (req, res) => {
    try {
      const profile = await CareAssistantProfile.findOne({ user: req.user._id }).select('_id').lean();
      if (!profile) return res.status(404).json({ success: false, message: 'Care assistant profile not found' });
...
// PATCH '/:id/care/start'
export const patchByIdCareStart = asyncHandler(async (req, res) => {
    try {
      const profile = await CareAssistantProfile.findOne({ user: req.user._id }).select('_id').lean();
      if (!profile) return res.status(404).json({ success: false, message: 'Care assistant profile not found' });
      const booking = await Booking.findOne({ _id: req.params.id, careAssistant: profile._id });
...
export const patchByIdCareComplete = asyncHandler(async (req, res) => {
    try {
      const profile = await CareAssistantProfile.findOne({ user: req.user._id }).select('_id').lean();
      if (!profile) return res.status(404).json({ success: false, message: 'Care assistant profile not found' });
      const booking = await Booking.findOne({ _id: req.params.id, careAssistant: profile._id });
...
// PATCH '/care/location'
export const patchCareLocation = asyncHandler(async (req, res) => {
    try {
...
export const postByIdCareJoinRide = asyncHandler(async (req, res) => {
    try {
...
export const patchByIdCareReachedJp = asyncHandler(async (req, res) => {
    try {
...
export const patchByIdCareRideStatus = asyncHandler(async (req, res) => {
    try {
...
export const getByIdCareTrackingSnapshot = asyncHandler(async (req, res) => {
    try {
...
export const getHospitalUpcoming = asyncHandler(async (req, res) => {
  try {
...
export const patchByIdHospitalConfirm = asyncHandler(async (req, res) => {
  try {
...
export const getHospitalByHospitalIdOps = asyncHandler(async (req, res) => {
    try {
...
export const getHospitalByHospitalIdValidOps = asyncHandler(async (req, res) => {
    try {
      const { hospitalId } = req.params;
...
// GET '/doctor/ops'
export const getDoctorOps = asyncHandler(async (req, res) => {
    try {
...
export const getDoctorOpsByOpNumber = asyncHandler(async (req, res) => {
    try {
      const doctorProfile = await DoctorProfile.findOne({ user: req.user._id }).select('_id').lean();
      if (!doctorProfile) return res.status(404).json({ success: false, message: 'Doctor profile not found' });
...
export const patchByIdOpComplete = asyncHandler(async (req, res) => {
  try {
    const doctorProfile = await DoctorProfile.findOne({ user: req.user._id }).select('_id').lean();
    if (!doctorProfile) return res.status(404).json({ success: false, message: 'Doctor profile not found' });
...
// GET '/op/:opNumber'
export const getOpByOpNumber = asyncHandler(async (req, res) => {
    try {
...
export const getOpByOpNumberFollowUps = asyncHandler(async (req, res) => {
  try {
...
export const getOpByOpNumberDownload = asyncHandler(async (req, res) => {
  try {
    const filter = { opNumber: req.params.opNumber };
    if (req.user.role === 'customer') filter.patient = req.user._id;
...
export const getAdminBookingsStats = asyncHandler(async (req, res) => {
    try {
...
export const getAdminBookingsExport = asyncHandler(async (req, res) => {
  try {
...
export const getAdminBookingsById = asyncHandler(async (req, res) => {
    try {
      const booking = await Booking.findById(req.params.id)
        .populate('customer', 'name phone email')
...
export const patchAdminBookingsByIdStatus = asyncHandler(async (req, res) => {
  try {
...
if (status === 'cancelled') {
      ...
      // Doctor REJECT → cancel any linked consultation too, so it doesn't dangle in 'scheduled'/'waiting'.
      if (booking.consultationSessionId) {
        await Consultation.findByIdAndUpdate(booking.consultationSessionId, {
          $set: {
            status:       'cancelled',
            cancellation: { cancelledBy: 'doctor', cancelledById: req.user._id, reason: note || 'Booking rejected by doctor', cancelledAt: new Date() },
            updatedBy:    req.user._id,
          },
        }).catch(e => console.error('[admin/status] consultation cancel failed:', e.message));
      }
    }
...
// Doctor ACCEPT (status → confirmed): online consult → auto-create Consultation + link.
    // In-person consult → just accept, no Consultation needed (OP record handles that path).
    const ONLINE_CONSULT_TYPES = ['video', 'audio'];
    const consultTypeMap = { video: 'video', inPerson: 'in_person', homeVisit: 'home_visit' };
    const mappedConsultType = consultTypeMap[booking.consultationType] || null;

    if (status === 'confirmed' && booking.doctor && !booking.consultationSessionId) {
      if (mappedConsultType && ONLINE_CONSULT_TYPES.includes(mappedConsultType)) {
        try {
          ...
          const consultation = new Consultation({
            booking:          booking._id,           // FIX: schema field is `booking`, not `bookingId`
            patient:          booking.customer,
            doctor:           booking.doctor,
            hospital:         booking.hospital || null,
            consultationType: mappedConsultType,       // must be one of CONSULTATION_TYPES
            scheduledAt:      booking.scheduledAt,      // FIX: schema field is `scheduledAt`, required
            status:           'scheduled',              // FIX: 'scheduled' valid, old 'scheduled' ok but keep explicit
            agora: {                                    // FIX: roomId/hostToken etc live under `agora`, not top-level
              channelName:     roomId,
              rtmChannelName:  meetingId,
              doctorRtcToken:  hostToken,
              patientRtcToken: participantToken,
            },
      ...
      // in-person / home-visit: nothing to create here — createOrSendOpOnConfirmation (called below) handles it.
    }
...
export const patchAdminBookingsByIdDestination = asyncHandler(async (req, res) => {
  try {
...
export const getAdminBookingsByIdNearbyCareAssistants = asyncHandler(async (req, res) => {
    try {
...
export const getAdminBookingsByIdNearbySoloDrivers = asyncHandler(async (req, res) => {
    try {
...
export const getAdminBookingsByIdNearbyTransportPartners = asyncHandler(async (req, res) => {
    try {
      const booking = await Booking.findById(req.params.id).select('patientLocation').lean();
      if (!booking) return res.status(404).json({ success: false, message: 'Booking not found' });
      const pickupCoords = booking.patientLocation?.coordinates;
...
export const getAdminBookingsByIdNearbyHospitals = asyncHandler(async (req, res) => {
    try {
      const booking = await Booking.findById(req.params.id).select('patientLocation').lean();
      if (!booking) return res.status(404).json({ success: false, message: 'Booking not found' });
      const coords = booking.patientLocation?.coordinates;
...
export const postAdminBookingsByIdAssignSoloDriver = asyncHandler(async (req, res) => {
  try {
...
export const postAdminBookingsByIdAssignTransportPartner = asyncHandler(async (req, res) => {
  try {
...
export const postAdminBookingsByIdAssignCareAssistant = asyncHandler(async (req, res) => {
  try {
...
export const postAdminBookingsByIdAssignHospital = asyncHandler(async (req, res) => {
  try {
...
export const patchAdminBookingsByIdReassignCare = asyncHandler(async (req, res) => {
  try {
...
export const postAdminBookingsByIdRefund = asyncHandler(async (req, res) => {
  try {
...
// GET '/admin/ops'
export const getAdminOps = asyncHandler(async (req, res) => {
    try {
...
export const patchAdminOpsByIdStatus = asyncHandler(async (req, res) => {
  try {
...
export const getAdminSosActive = asyncHandler(async (req, res) => {
  try {
...
export const patchAdminSosBySosEventIdResolve = asyncHandler(async (req, res) => {
  try {
...
export const getAdminDestinationAuditByBookingId = asyncHandler(async (req, res) => {
  try {
