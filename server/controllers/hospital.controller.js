...
// FIX: hospital-filter and search-filter both used to write to filter.$or,
  // so combining ?hospital=...&search=... silently dropped the hospital
  // constraint (second assignment overwrote the first). Combine both under
  // $and instead so each condition is actually enforced together.
  const orConditions = [];
  if (req.query.hospital) {
    ...
    orConditions.push({
      $or: [
        { primaryHospital: new mongoose.Types.ObjectId(req.query.hospital) },
        { otherHospitals:  new mongoose.Types.ObjectId(req.query.hospital) },
      ],
    });
  }
  if (req.query.search) {
    const regex = new RegExp(req.query.search, 'i');
    orConditions.push({ $or: [{ biography: regex }, { achievements: regex }] });
  }
  if (orConditions.length) {
    filter.$and = orConditions;
  }

// Handle cross-collection Name Sorting AND lowestFee sorting via aggregation.
  // FIX: lowestFee used to sort on raw DoctorProfile.fees.inPersonFee — but
  // per resolveEffectivePricing, that's only the real price for doctor-owner
  // doctors. Hospital-manager doctors' true price is
  // Hospital.consultationPricing.inPersonFee. Now computed + sorted correctly.
  if (req.query.sort === 'name' || req.query.sort === 'lowestFee') {
    const pipeline = [
      { $match: filter },
      { $lookup: { from: 'users', localField: 'user', foreignField: '_id', as: 'user' } },
      { $unwind: '$user' },
      { $lookup: { from: 'hospitals', localField: 'primaryHospital', foreignField: '_id', as: 'primaryHospital' } },
      { $unwind: { path: '$primaryHospital', preserveNullAndEmptyArrays: true } },
    ];

    if (req.query.sort === 'lowestFee') {
      pipeline.push({
        $addFields: {
          effectiveInPersonFee: {
            $cond: [
              { $eq: ['$primaryHospital.managementModel', 'hospital-manager'] },
              { $ifNull: ['$primaryHospital.consultationPricing.inPersonFee', 999999] },
              { $ifNull: ['$fees.inPersonFee', { $ifNull: ['$fees.consultationFee', 999999] }] },
            ],
          },
        },
      });
      pipeline.push({ $sort: { effectiveInPersonFee: 1 } });
    } else {
    ...
    pipeline.push(
      { $skip: skip },
      { $limit: limit },
      { $project: { 'kyc.aadhaarNumber': 0, 'kyc.panNumber': 0, adminNotes: 0, 'bankDetails.accountNumber': 0, contractUrl: 0, platformFee: 0, stats: 0 } }
    );

    const [doctors, total] = await Promise.all([
      DoctorProfile.aggregate(pipeline),
      DoctorProfile.countDocuments(filter)
    ]);

    const sanitized = doctors.map(sanitizeDoctorPricing);
    return res.json({
      success: true, count: sanitized.length, total, page, pages: Math.ceil(total / limit), data: sanitized,
    });
  }

  // Default sorting maps
  const sortMap = {
    rating:     { 'rating.averageRating': -1 },
    experience: { experienceYears: -1 },
    newest:     { createdAt: -1 },
  };
  const sortObj = sortMap[req.query.sort] || { 'rating.averageRating': -1 };
...
if (req.query.rating !== undefined) {
    const ratingNum = parseFloat(req.query.rating);
    if (!isNaN(ratingNum)) filter['rating.averageRating'] = { $gte: ratingNum };
  }
  if (req.query.consultationType) filter[`consultationTypes.${req.query.consultationType}`] = true;
...
// Handle cross-collection Name Sorting AND lowestFee sorting via aggregation.
  // FIX: same effective-price issue as getAllDoctors.
  if (req.query.sort === 'name' || req.query.sort === 'lowestFee') {
    const pipeline = [
      { $match: finalFilter },
      { $lookup: { from: 'users', localField: 'user', foreignField: '_id', as: 'user' } },
    ...
    const sanitized = doctors.map(sanitizeDoctorPricing);
    return res.json({
      success: true, specialization: normalizedSpec, count: sanitized.length, total, page, pages: Math.ceil(total / limit), data: sanitized,
    });
  }
  ...
  const sortObj = sortMap[req.query.sort] || { 'rating.averageRating': -1, experienceYears: -1 };
  const [doctors, total] = await Promise.all([
...
const searchDoctors = asyncHandler(async (req, res) => {
  ...
  const matchingUsers = await User.find({ role: 'doctor', name: regex }).select('_id').lean();
  ...
  const filter = {
    ...
    isVerified:        true,
    partnershipStatus: 'Active',
    $or: [ { user: { $in: userIds } }, { biography: regex }, { achievements: regex } ],
  };
  ...
  if (req.query.sort === 'name' || req.query.sort === 'lowestFee') {
    ...
    const sanitized = doctors.map(sanitizeDoctorPricing);
    return res.json({
      success: true, query: q, count: sanitized.length, total, page, pages: Math.ceil(total / limit), data: sanitized,
    });
  }
  ...
  const sanitized = doctors.map(sanitizeDoctorPricing);
  res.json({ success: true, query: q, count: sanitized.length, total, page, pages: Math.ceil(total / limit), data: sanitized });
});
...
const alreadyLinked = hospital.linkedDoctors.some((id) => id.toString() === doctorId);
  if (alreadyLinked) {
  ...
  } else if (!doctor.otherHospitals.some((id) => id.toString() === hospitalId)) {
...
// GET '/nearby'
export const getNearby = asyncHandler(getNearbyHospitals);

// GET '/search'
export const getSearch = asyncHandler(searchHospitals);

// GET '/'
export const get = asyncHandler(getAllHospitals);

// GET '/slug/:slug'
export const getSlugBySlug = asyncHandler(getHospitalBySlug);

// GET '/doctors/nearby'
export const getDoctorsNearby = asyncHandler(getNearbyDoctors);

// GET '/doctors/search'
export const getDoctorsSearch = asyncHandler(searchDoctors);

// GET '/doctors'
export const getDoctors = asyncHandler(getAllDoctors);

// GET '/doctors/specialization/:spec'
export const getDoctorsSpecializationBySpec = asyncHandler(getDoctorsBySpecialization);

// GET '/doctors/by-hospital/:hospitalId'
export const getDoctorsByHospitalByHospitalId = asyncHandler(getDoctorsByHospital);

// GET '/doctors/me'
export const getDoctorsMe = asyncHandler(getMyDoctorProfile);

// GET '/doctors/me/hospitals'
export const getDoctorsMeHospitals = asyncHandler(getMyManagedHospitals);

// GET '/doctors/me/pricing'
export const getDoctorsMePricing = asyncHandler(getMyEffectivePricing);

// POST '/doctors'
export const postDoctors = asyncHandler(createDoctorProfile);

// PUT '/doctors/:id/profile'
export const putDoctorsByIdProfile = asyncHandler(updateDoctorProfile);

// PUT '/doctors/:id/settings'
export const putDoctorsByIdSettings = asyncHandler(updateDoctorSettings);

// PUT '/doctors/:id/availability'
export const putDoctorsByIdAvailability = asyncHandler(updateDoctorAvailability);

// PUT '/doctors/:id/bank'
export const putDoctorsByIdBank = asyncHandler(updateDoctorBankDetails);

// PUT '/doctors/:id/kyc'
export const putDoctorsByIdKyc = asyncHandler(updateDoctorKyc);

// GET '/doctors/:id/stats'
export const getDoctorsByIdStats = asyncHandler(getDoctorStats);

// POST '/doctors/:id/photo'
export const postDoctorsByIdPhoto = asyncHandler(uploadDoctorPhoto);

// POST '/doctors/:id/signature'
export const postDoctorsByIdSignature = asyncHandler(uploadDoctorSignature);

// PUT '/doctors/:id/security'
export const putDoctorsByIdSecurity = asyncHandler(updateDoctorSecurity);

// PUT '/doctors/:id/platform-fee'
export const putDoctorsByIdPlatformFee = asyncHandler(updateDoctorPlatformFee);

// PUT '/doctors/:id/partnership'
export const putDoctorsByIdPartnership = asyncHandler(updateDoctorPartnership);

// PUT '/doctors/:id/kyc/verify'
export const putDoctorsByIdKycVerify = asyncHandler(verifyDoctorKyc);

// PUT '/doctors/:id/toggle'
export const putDoctorsByIdToggle = asyncHandler(toggleDoctorActive);

// POST '/doctors/:id/resend-credentials'
export const postDoctorsByIdResendCredentials = asyncHandler(resendDoctorCredentials);

// DELETE '/doctors/:id'
export const deleteDoctorsById = asyncHandler(deleteDoctorProfile);

// GET '/doctors/:id'
export const getDoctorsById = asyncHandler(getDoctorById);

// POST '/'
export const post = asyncHandler(createHospital);

// PUT '/:id/profile'
export const putByIdProfile = asyncHandler(updateHospitalProfile);

// PUT '/:id/settings'
export const putByIdSettings = asyncHandler(updateHospitalSettings);

// PUT '/:id/security'
export const putByIdSecurity = asyncHandler(updateHospitalSecurity);

// PUT '/:id/consultation-pricing'
export const putByIdConsultationPricing = asyncHandler(updateHospitalConsultationPricing);

// PUT '/:id/platform-fee'
export const putByIdPlatformFee = asyncHandler(updateHospitalPlatformFeeOverride);

// POST '/:id/resend-credentials'
export const postByIdResendCredentials = asyncHandler(resendHospitalManagerCredentials);

// POST '/:id/images'
export const postByIdImages = asyncHandler(uploadHospitalImages);

// DELETE '/:id/images/:imageIndex'
export const deleteByIdImagesByImageIndex = asyncHandler(deleteHospitalImage);

// PUT '/:id/location'
export const putByIdLocation = asyncHandler(updateHospitalLocation);

// POST '/:id/doctors/:doctorId'
export const postByIdDoctorsByDoctorId = asyncHandler(linkDoctorToHospital);

// DELETE '/:id/doctors/:doctorId'
export const deleteByIdDoctorsByDoctorId = asyncHandler(unlinkDoctorFromHospital);

// PUT '/:id/verify'
export const putByIdVerify = asyncHandler(verifyHospital);

// PUT '/:id/toggle'
export const putByIdToggle = asyncHandler(toggleHospitalActive);

// DELETE '/:id'
export const deleteById = asyncHandler(deleteHospital);

// GET '/:id/pricing'
export const getByIdPricing = asyncHandler(getHospitalEffectivePricing);

// GET '/:id'
export const getById = asyncHandler(getHospitalById);
