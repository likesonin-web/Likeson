...
export default function PinnedConversationsPage() {
  const router = useRouter();
  const conversations = useSelector(selectConversations);
  const pinnedOnly = conversations.filter((i) => i.isPinned);
  ...
  return (
    <div className="flex flex-col h-[100dvh]">
      <header className="flex items-center gap-3 px-4 py-3 border-b border-base-300">
        ...
        <Pin size={16} className="text-base-content/50" />
        <h1 className="text-sm font-bold">Pinned Conversations</h1>
      </header>
      <div className="flex-1 overflow-y-auto scrollbar-thin">
        <ConversationList items={pinnedOnly} />
      </div>
    </div>
  );
}
