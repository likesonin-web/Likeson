      ...
      <SectionCard>
        ...
        <div className="space-y-2.5">
          {[
           ...
           {
              label: "Consultation fee set",
              // Hospital-managed doctor: fee lives on primaryHospital.consultationPricing, not doctor.fees.
              done: profile?.primaryHospital?.managementModel === "hospital-manager"
                ? (profile?.primaryHospital?.consultationPricing?.inPersonFee ?? 0) > 0
                : (profile?.fees?.inPersonFee ?? 0) > 0,
            },
...
const isHospitalManaged = profile?.primaryHospital?.managementModel === "hospital-manager";
  ...
  const save = async () => {
    if (!profile?._id) return;
    // Guard: doctor-owner only. managementModel is fixed on the hospital,
    // doctor never changes it — mirrors backend assertFeesAllowed.
